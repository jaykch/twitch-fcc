    ...
    var _generateUsers = function () {
        ...
        for (var i = 0; i < apiCall.length; i++) {
            if (apiCall[i].display_name) {
                ...
                imageUrl = "https://static-cdn.jtvnw.net/jtv_user_pictures/" + apiCall[i].display_name.toLowerCase() + "-profile_image-9021dccf9399929e-300x300.jpeg";
                log(imageUrl);
                ...
                users.push(new user(apiCall[i].stream.display_name, apiCall[i].stream.url, "streaming", apiCall[i].stream.status, "active", apiCall[i].stream.logo.toLowerCase()));
            } else {
                log("Unknown User");
        ...
        for (var i = 0; i < users.length; i++) {
            if (users[i].accountStatus == "active") {
                ...
                if (users[i].onlineStatus == "offline") {
                    userOutput.innerHTML += "<li class='user-offline'>"
                        + "<img src='" + users[i].logo + "' alt='profile' class='user-logo'>"
                        + "<a class='user-name' href='"
                        + users[i].url + "' target='_blank'>" + users[i].name + "</a><br>"
                        + "<span class='online-status'>Status: " + users[i].onlineStatus + " </span><br>"
                        + "<span class='stream-status'>Stream Status: " + users[i].streamStatus + "</span>"
                        + "</li>";
                } else {
                    userOutput.innerHTML += "<li class='user-online'>"
                        + "<img src='" + users[i].logo + "' alt='profile' class='user-logo'>"
                        + "<a class='user-name' href='"
                        + users[i].url + "' target='_blank'>" + users[i].name + "</a><br>"
                        + "<span class='online-status'>Status: " + users[i].onlineStatus + " </span><br>"
                ...
                userOutput.innerHTML += "<li class='user-offline'>" + "<span class='user-name'>" + users[i].name + " (404)</span></li>"
            }
        }
    };
